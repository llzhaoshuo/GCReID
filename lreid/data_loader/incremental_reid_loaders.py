...
class IncrementalReIDLoaders:
    ...
    def _load(self):
        ...
        # train_samples_1 = self._get_train_samples(self.config.train_dataset)
...
# 上面的for的作用：每个域的训练集中选500个identity
    ...
    def _get_uniform_incremental_iter(self, samples, transform, p, k, pid_list):
        ...
        # print("current transform is -------------------------------", transform)
        
        dataset = IncrementalReIDDataSet(samples, self.total_step, transform=transform, image_size=self.config.image_size)
        # dataset_init = IncrementalReIDDataSet_init(samples,self.total_step, transform=transform, image_size=self.config.image_size)
        # dataset.samples = dataset.samples+dataset_init.samples # 增强的样本+原始样本
        # embed()
        ...
        # loaders_init = data.DataLoader(dataset_init, batch_size=p * k, num_workers=8, drop_last=False,
        #                          sampler=ClassUniformlySampler4Incremental(dataset, class_position=1, k=k, pid_list=pid_list))
        
        iters = IterLoader(loader)
        # iters_init = IterLoader(loaders_init)
        return iters#, iters_init
