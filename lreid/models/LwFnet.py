...
class LwFNet(nn.Module):
    ...
    def __init__(self, class_num_list, pretrained=True):
        ...
        for step, num in enumerate(self.class_num_list):
            self.classifier_dict[f'step:{step}'] = BNClassifier(self.feature_dim, num) # 多少个identity就是多少分类问题

        # self.pid_num = 2500
        # self.crossentropy = CrossEntropyLabelSmooth(self.pid_num)

    def forward(self, x, current_step=0):
        if isinstance(current_step, list): # 如果current_step = [0,1,2] 列表格式的.说明是增量训练
            
            feature_maps = self.backbone(x) # backbone的输出
            cls_score_list = []
            features = self.encoder_feature(feature_maps) # 加了avg
            for c_s in current_step:
                bned_features, cls_score = self.classifier_dict[f'step:{c_s}'](features) # 分类器的输出和置信度
                cls_score_list.append(cls_score)
            if self.training:
                return features, cls_score_list, feature_maps #cls_score_list 多个step的分类结果
            else:
                return bned_features, feature_maps
        else: # 如果current_step = 0 
            feature_maps = self.backbone(x)
            features = self.encoder_feature(feature_maps) #features.shape : torch.Size([32, 2048])
