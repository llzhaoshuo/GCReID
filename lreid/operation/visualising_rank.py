...
from lreid.tools import make_dirs
from IPython import embed

def visualize_one_ranked_results(query, distmat, dataset, save_dir='', topk=20, sort='descend', mode='inter-camera', only_show=None):
# def visualize_one_ranked_results(query, distmat, dataset, save_dir='', topk=10, sort='descend', mode='inter-camera', only_show=None):
    ...
    # query, gallery = dataset[0], dataset[1]
    query = query
    gallery = dataset[1]
    ...
    def cat_imgs_to(image_list, hit_list, text_list, target_dir):
        ...
        for img, hit, text in zip(image_list, hit_list, text_list):
            img = Image.open(img).resize((64, 128)) # 行人大小
            # img = Image.open(img).resize((128, 128)) #车辆大小
    ...
    for q_idx in range(num_q):
        ...
        qimg_path, qpid, qcamid = query[0:3]
        qpid = int(qpid)
        ...
        counts += 1
        print(counts, qdir)
        new_im = cat_imgs_to(image_list, hit_list, text_list, qdir)
        return qdir, new_im
